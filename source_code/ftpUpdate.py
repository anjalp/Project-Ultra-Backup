...
def ftpManualUpdate(ftpAdress, port, userName, password, backup_to):
    trying = 0       #this function make engae the while loop to login continuously.
    backup_folder = ''     #folder in the FTP device to backup.
    connStatus = 'no'     #connection status of the login, if connected then connSatus = 'ok'
    userExit = 0   #when user want to exit form the browse FTP Device storage.
    mapFTP = {}     #store the FTP device storage folder, subfolder anf file map.
    noFolder = 0     
    fileCount = 0
    loop = True
    ftp = FTP()
        ...
        try: 
            while toDo!='exit':
                ...
                if option.isdigit()==True:
                    ...
                    mainloc = backup_folder.replace("/storage/", "")   #mapping the folder
                    loc = mainloc
                    mapFTP[mainloc] = {}
                    while loop==True:
                        noFolder = 0
                        ...
                        for f in files:    
                            if (f[52:])[0]!='.':
                                if loc + "/" + f[52:] in mapFTP:
                                    ...
                                    mapFTP[loc][fileCount] = []    # [filename, filesize, filemodfifed date]
                                    size = f.split()[4]
                                    datestr = ' '.join(f.split()[5:8])   #taking the date and time from list
                                    dateI = parser.parse(datestr)     # converting to y-m-d h:m:s
                                    dateIe = int(dateI.timestamp())    # converting datetime to epoch
                    ...
                    mapFTP = updateEngine(mapFTP, backup_to, backup_folder)  #this function removes all the previously present files and only keeps the new files
                    numFolder = 0
                    numFiles = 0
                    ...
                    if numFiles!=0 or numFolder!=0:
                        ...
                        for eachFolder in mapFTP:
                            ...
                            try:
                                ...
                                errorReport["Exception: "] = str(e)
                                errorSave(backup_to, errorReport, backup_to)
                            newFolder = ''    #code to shoten the folder name
                            ...
                            for eachFile in mapFTP[eachFolder]:
                                if len(backup_to + "//" + "AndroidUB" + "//" + "UB_" + todayDate + "//" + folderPC + "//" + mapFTP[eachFolder][eachFile][0]) > 250:
                                    x = mapFTP[eachFolder][eachFile][0].split(".")    #this code to shoten the file name.
                                    file = ''
                                    for joinFile in x[:-1]:
                                        if file=='':
                                            file = joinFile
                    ...
                    print("Please type a valid response./n")
        except Exception as e:
