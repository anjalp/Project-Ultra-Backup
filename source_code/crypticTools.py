...
def viewUB(ubmapDirectory):
    ...
    if os.path.isdir(ubmapDirectory)==False:
        ...
        if presentCount==0:
            ...
            return
        print("\n   --Enter the number corresponding to the UBmap file to view it.")     #print the ubmap files present in the selected folder
        print('')
        for eachUB in ubList:
            print("        " + str(eachUB) + " : " + os.path.basename(ubList[eachUB]))
        selectUBmapFile = int(input("\n   --Enter the number: "))   #select the ubmap to view
        with open(ubList[selectUBmapFile], 'r') as ubmapFile:    #open the selected ubmap file
            dataUB = copy.deepcopy(json.load(ubmapFile))
            ubmapFile.close()
        for data in dataUB:    #deelting the z_n an the k in the data.
            dataUB[data].pop("z_n")
            dataUB[data].pop("k")
                ...
                for eachData in dataUB:
                    for eachFile in dataUB[eachData]["fi_s"]:
                        if dataUB[eachData]["fi_s"][eachFile]==fileToSearch:
                            ...
                            foundList[foundCount] = {}       #contains the: UBmap name, file location, file size, (a, c, m)time, z_n name, key
